...
function appendNewCard(parentElement) {
  ...
  let childrenDivs = '<div class="card-down"></div><div class="card-up"></div>';
  card.innerHTML = childrenDivs;
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++){
    let card = {
      ...
      assignImg: function(){
        this.element.classList.add(this.imageClass);
      }
    };
    card.assignImg()
    cardList.push(card);
...
function incrementCounter(counterName, parentElement) {
  ...
  if (counters.hasOwnProperty(counterName) === false){
...
function onCardFlipped(newlyFlippedCard) {
  ...
  if (lastCardFlipped === null){
    ...
    // calls random glow function and uses that value to set the css variable to a random color
    document.documentElement.style.setProperty('--random-glow', randomGlowColor());
...
//returns string literal containing css for a random rgba color value
function randomGlowColor(){
  red = randomAnalouge();
  green = randomAnalouge();
  blue = randomAnalouge();
  return `rgba(${red},${green},${blue}, .8)`
}
//returns random number between 0 and 255
function randomAnalouge(){
  // use math random to grab random number between 0 and 255
